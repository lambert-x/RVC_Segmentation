...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B: int, H: int, W: int, fp32=True):
        ...
        y_embed = torch.arange(1, H + 1, dtype=torch.float32 if fp32 else torch.float16, device=device).unsqueeze(
            1).repeat(1, 1, W)
...
class ClassAttn(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x[:, 0]).unsqueeze(1).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1,
                                                                                                    3).contiguous()
        k = self.k(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).contiguous()
        ...
        x_cls = (attn @ v).transpose(1, 2).reshape(B, 1, C).contiguous()
...
class fan_base_16_p4_hybrid(FAN):
    def __init__(self, **kwargs):
        ...
        super(fan_base_16_p4_hybrid, self).__init__(
            patch_size=16, in_chans=3, num_classes=1000, embed_dim=448, depth=depth, backbone=backbone, out_idx=15,
            feat_downsample=False,
            num_heads=8, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
            act_layer=None, norm_layer=None, cls_attn_layers=2, use_pos_embed=True, eta=1., tokens_norm=True)
...
class fan_base_18_p16_224(FAN):
    def __init__(self, **kwargs):
        ...
        super(fan_base_18_p16_224, self).__init__(
            ...
            num_heads=8, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., se_style=True,
            out_idx=16,
            act_layer=None, norm_layer=None, cls_attn_layers=2, use_pos_embed=True, eta=1., tokens_norm=True,
            use_checkpoint=False)
